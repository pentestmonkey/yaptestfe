class CredentialsController < ApplicationController
  ...
  def index
   ...
   if not params[:host_id_v].nil?
     ...
     @page_title="Credentials #{@host.ip_address} in area #{@host.test_area_name}"
   ...
   if not params[:type_v].nil?
     where[:credential_type_name] = params[:type_v]
     @url_params[:type_v] = params[:type_v]
   end

   if not params[:domain_v].nil?
     where[:domain] = params[:domain_v]
     @url_params[:domain_v] = params[:domain_v]
   end
  ...
  if session[:test_area].nil?
    @record_count = Credential.find(:all, :conditions => where).length
    @pager = Paginator.new(@record_count, session[:tablen]) do |offset, per_page|
      Credential.find(:all, :order => order_clause, :limit => per_page, :offset => offset, :conditions => where)
    end
  else
    @record_count = Credential.find_all_by_test_area_id(session[:test_area], :conditions => where).length
    @pager = Paginator.new(@record_count, session[:tablen]) do |offset, per_page|
      Credential.find_all_by_test_area_id(session[:test_area], :order => order_clause, :limit => per_page, :offset => offset, :conditions => where)
    end
  end
